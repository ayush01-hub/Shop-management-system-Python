...
db=mysql.connector.connect(user='user-name',passwd="your-password", host="localhost")
...
db=mysql.connector.connect(user="user-name",passwd="your-password", host="localhost", database='shop')
my_cursor=db.cursor()
#query to creating table products
query="CREATE TABLE IF NOT EXISTS products (date VARCHAR(10), prodame VARCHAR(20), prodPrice VARCHAR(50))"
my_cursor.execute(query)#executing query
...
def prodtoTable():
    #getting the user input of product details
    pname=prodName.get()
    price=prodPrice.get()
    dt=date.get()
    ...
    #query to add product detail to the table
    query="INSERT INTO products(date,prodName,podPrice)  VALUES(%s, %s, %s)"
    details=(dt, pname, price)

    #Executing query and showing the popup message
    try:
        cursor.execute(query,details)
        db.commit
        messagebox.showinfo('Success',"Product added successfully")
    except Exception as e:
        print("The execption is:",e)
        message.showinfo("Error","Trouble adding data into Database")
    wn.destroy()

#Function to get details of product to be added
def addProd():
    ...
    headingFrame1 = Frame(wn,bg='LightBlue1',bd=5).place(relx=0.25,rely=0.1,relwidth=0.5,relheight=0.13)
    headingLabel = Label(headingFrame1, text="Add a Product", fg='grey19', font=('Courier',15,'bold')).place(relx=0,rely=0, relwidth=1, relheight=1)

    labelFrame = Frame(wn).place(relx=0.1,rely=0.4,relwidth=0.8,relheight=0.4)
        
    # Getting Date
    lable1 = Label(labelFrame,text="Date : ", fg='black').place(relx=0.05,rely=0.3, relheight=0.08)
    date = Entry(labelFrame).place(relx=0.3,rely=0.3, relwidth=0.62, relheight=0.08)
        
    # Product Name
    lable2 = Label(labelFrame,text="Product Name : ", fg='black').place(relx=0.05,rely=0.45, relheight=0.08)
    prodName = Entry(labelFrame).place(relx=0.3,rely=0.45, relwidth=0.62, relheight=0.08)
        
    # Product Price
    lable3 = Label(labelFrame,text="Product Price : ", fg='black').place(relx=0.05,rely=0.6, relheight=0.08)
    prodPrice = Entry(labelFrame).place(relx=0.3,rely=0.6, relwidth=0.62, relheight=0.08)

    #Add Button
    Btn = Button(wn,text="ADD",bg='#d1ccc0', fg='black',command=prodtoTable).place(relx=0.28,rely=0.85, relwidth=0.18,relheight=0.08)
    Quit= Button(wn,text="Quit",bg='#f7f1e3', fg='black',command=wn.destroy).place(relx=0.53,rely=0.85, relwidth=0.18,relheight=0.08)
...
def delProd():
    ...
    headingFrame1 = Frame(wn,bg="misty rose",bd=5).place(relx=0.25,rely=0.1,relwidth=0.5,relheight=0.13)
    headingLabel = Label(headingFrame1, text="Delete Product", fg='grey19', font=('Courier',15,'bold')).place(relx=0,rely=0, relwidth=1, relheight=1)
    
    labelFrame = Frame(wn).place(relx=0.1,rely=0.3,relwidth=0.8,relheight=0.5)
        
    # Product Name to Delete
    lable = Label(labelFrame,text="Product Name : ", fg='black').place(relx=0.05,rely=0.5)
        
    prodName = Entry(labelFrame).place(relx=0.3,rely=0.5, relwidth=0.62)
    
    #Delete Button
    Btn = Button(wn,text="DELETE",bg='#d1ccc0', fg='black',command=removeProd).place(relx=0.28,rely=0.9, relwidth=0.18,relheight=0.08)
    
    Quit = Button(wn,text="Quit",bg='#f7f1e3', fg='black', command=wn.destroy).place(relx=0.53,rely=0.9, relwidth=0.18,relheight=0.08)
...
headingFrame1.place(relx=0.2,rely=0.1,relwidth=0.6,relheight=0.16)
headingLabel = Label(headingFrame1, text="Welcome Here \n Shop Management System", fg='grey19', font=('Courier',15,'bold'))
headingLabel.place(relx=0,rely=0, relwidth=1, relheight=1)

#Button to add new product, delete a product, view all product,  Customer and generate bill
Button1 = Button(wn, text="Add A Product", font=(20),bg='LightBlue1', fg='black', width=20, height=2,command=addProd).place(x=270, y=175)

button2 = Button(wn,text="Delete a Product",font=(20),bg='misty rose', fg='black',width=20,height=2,command=delProd).place(x=270,y=255)

button3 = Button(wn,text="View Products",font=(20),bg='old lace', fg='black',width=20,height=2,command=viewProds).place(x=270,y=335)

button4 = Button(wn,text="Customer",font=(20),bg='lavender blush2', fg='black', width=20,height=2,command = newCust).place(x=270,y=415)
